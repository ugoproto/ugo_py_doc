...
Make a choice depending on the variable type. However, if calling a 'string' (text, NLP) must be done with `%s`, 'numbers' is done with `%r` and `%s` (possibly with `%s`). The use and effects of the different calls is explained in Exercises 6 and 21.
...
- Before running this script, create a text file in the shell.
- Type `"Test file thing" > test.txt`
    - It will pipe the text into the file 'test.txt'
- Then type `cat test.txt`
    - It will print the content of 'test.txt' on the screen
- Finally, 'test.txt' is now saved along the other scripts
- In the shell, you can run the current script using: `python ex15.py test.txt`
- With IPython, type: `%run ex15.py test.txt`
- Run the script without the extra argument `test.txt` to see.
...
```
    ...
    D:\Admin_Hugues\Documents\Notebooks\Learn Python the Hard Way\ex15.py in <module>()
...
- Load the script in IPython with `%load` to study (don't run it!).
- This type of script requires 'external' arguments when launched.
- In the script, always preceed the 'external' arguments, like `filename`, with `script`.
...
The script again (manual import or copy-paste).
...
First, let's load the script by typing `%load ex16_1.py`; the line will load the script, then turn to comment `# %load ex16_1.py`.
...
Now, let's run the external file in IPython (no need to type in the code).

Careful: the script requires an second argument (`filename`).
...
Now, let's truncate (empty, delete, erase, clear) the file, reopen it, and to read it.
...
- `target.read()`; read the whole file.
- `target.read(10)`; read the amount of bytes between the parentheses (1 byte = 1 character).
- `target.readline()`; read the first line or the file until the first `\n`.
- `target.readline(10)`; read 10 bytes..

First, load `script ex17_1.py`. This script needs two additional arguments.
...
- `*args` means indifinite number of arguments. All the arguments are is a list ('args').
...
Give different names to functions and arguments; not to get confuse too.
...
Two-dimentional list (above 2 dimension, it can become memory-intensive to compute!).
...
A dictionary has keys associated with values. Order does not matter. If you supply the key, you will get the value.
...
`print "%r" % stuff.get('color',None)`. If it's not existant, it will return 'None' or a value by default
...
Module `hashmap.py` works through a dictionary. Module `ex39_test.py` contains dictionaries (just like the above).

In Python, we would run module `ex39_test.py`. The module would begin by importing module `hashmap.py` to use its methods and perform operations.
...
Write the method the other around: `Class.class_function()`.
...
It looks like `Class.class_function()`.
...
- class X
- class X that is-a
- class X(Object): \__init\__(self, parameter):
- class X has-a \__init\__ that takes self and another parameter
- class X has-a function named that takes self and another parameters
- Set to an instance of class X
- From class X, get the function, and call it with parameters self and another parameter
- From class X, get the attribute and set it to
...
Use an existing project, Exercise 43 for example, and adapt it.

### 1

Storyboarding tool.

Create a program that would question the user on its data and advise him on what would be the best graphic to use. Frame by frame, the user could build a presentation knowing slide 1 should be a histogram, slide 2 a scatter plot, etc. 
...
In Linux, before running any script, be sure to set the path to the project directory. In Windows, set the path in the environment variables. Otherwise, Python won't find the scripts.
...
- Exercises, from 50 to 52, are done with web.py. The project is called 'Gothonweb'.
- However, install lpthw.web for the sake of the exercises (it is related to web.py).
    - `sudo pip install lpthw.web`
- Learn more about [web.py](http://webpy.org/).
- Project Gothonweb is based on the project skeleton (seen above).
    - 'bin' (the main script is `app.py`)
    - 'docs'.
    - 'tests'.
    - 'map'.
    - 'static' for images, css.
    - 'templates' for html.
- Run the project with `python bin/app.py`. It will launch the script in sub-dir 'map' and the rest of the web codes (html, css, etc.).
- Your browser makes a network connection to your own computer which is called localhost and is a standard way of saying "whatever my own computer is called on the network."
- It also uses port 8080 [http://localhost:8080/](http://localhost:8080/).
- Or [http://127.0.0.1:8080/](http://127.0.0.1:8080/).
- Once it connects, it makes an HTTP request to the `bin/app.py` application and asks for the URL, which is commonly the first URL on any website
- Consult the manual to find out more.

## Exercise 51, Getting Input From A Browser

Continues Exercise 50. Consult the manual to find out more.

## Exercise 52, The Start Of Your Web Game

Continues Exercises 50 and 51. Consult the manual to find out more.

### Wrap up

- Refactor Exercises 43 and 47, into the web frameworks (Exercises 50 and 51).
- Add tests from previous exercises (Exercises 47 to 51).
- Create a GitHub.
    - [Example](https://github.com/CreaturePhil/Learn-Python-the-Hard-Way).
    - [Example](https://github.com/bitsai/book-exercises/tree/master/Learn%20Python%20the%20Hard%20Way/projects/ex50).
- Improve the project with:
    - [CSS](https://github.com/mejarc/gothonweb/tree/master/static/css).
    - [JS](https://github.com/mejarc/gothonweb/tree/master/static/js).
    - [Images](https://github.com/mejarc/gothonweb/tree/master/static/img).
